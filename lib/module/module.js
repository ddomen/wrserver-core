...
class Module {
    ...
    makeController(connection, message, cnt) {
        if (cnt) {
            let cm = new cnt(connection, this.events, this.console, this._services, this._models, this._interceptors.get(component_1.Interceptor.Controller));
