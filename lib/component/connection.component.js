...
class Connection {
    ...
    onParsed(message) {
        ...
        if (mod) {
            ...
            this.interceptors.intercept(mod.constructor, { type: 'function', callback: (int) => { int.call(this); } }, { type: 'false', callback: null }, { type: 'null', callback: () => { this.digest(mod, message); } }, { type: 'any', callback: () => { this.digest(mod, message); } });
        }
        else {
            return this.bad('bad target');
        }
