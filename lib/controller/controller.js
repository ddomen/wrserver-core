...
*/
class Controller {
    constructor(connection, events, services, models, interceptors) {
    ...
    digest(message) {
        ...
        if (page) {
            return this.interceptors.intercept(this.constructor.name.toLowerCase() + '.' + pageStr, [message], { type: 'function', callback: (int) => int.call(this, page) }, { type: 'null', callback: () => this.callPage(page, message) }, { type: 'false', callback: null }, { type: 'any', callback: () => this.callPage(page, message) }) || Controller.BadDig;
        }
