...
class WRServer {
        ...
        if (modules.length) {
            modules.forEach(mod => {
                if (!this.modules.find(m => m.constructor == mod)) {
                    let nmod = new mod(this.events, new component_1.Console(mod.name, this.directory));
    ...
    initServices(services) {
        ...
        uniqueServices.forEach(x => {
            ...
            x.service = new x.type(this.directory, this.events, new component_1.Console(x.type.name, this.directory));
            this.services.push(x.service);
            this.interceptors.inject(x.service);
            if (!x.service.dependencies.length) {
                x.service.init();
            }
        });
        return this;
